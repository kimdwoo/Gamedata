...
namespace SwordEnhancement
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            double successRate, destructionRate, enhancementCost, money; // 파괴 확률 초기값은 0.3%에서 시작
            ...
            if (TryLoadUserData(userId, out enhancementLevel, out successRate, out money, out destructionRate, out enhancementCost))
            {
                ...
                if (money < enhancementCost)
                {
                    ...
                    Console.Write($"판매 가격: {sellPrice:F1}원, ");

                    Console.ForegroundColor = ConsoleColor.Yellow;
                    Console.WriteLine($"남은 돈: {money:F1}원");

                    Console.WriteLine("\n강화 비용이 부족합니다. 돈을 충전하시거나 검을 판매하실 수 있습니다.");
                    Console.WriteLine("\n 돈을 충전하시려면 1000을 입력하세요. (종료: n)");
                    Console.WriteLine("\n돈을 충전하시려면 10000을 입력하세요. (종료: n)");
                    Console.WriteLine($"\n검을 판매하시겠습니까? 판매가격: {sellPrice} (판매: s)");
            ...
            static void AskForAction(ref double money, double enhancementCost, string userId, int enhancementLevel, double successRate, double destructionRate, int sellPrice)
            {
                Console.ForegroundColor = ConsoleColor.Blue;
                Console.Write($"현재 강화 비용: {enhancementCost:F1}원, ");
                Console.ForegroundColor = ConsoleColor.Green;
            ...
            static void SellSword(ref double money, int enhancementLevel, double successRate, double destructionRate, double enhancementCost, out int newEnhancementLevel, out double newSuccessRate, out double newDestructionRate, out double newEnhancementCost)
            {
